namespace Nzl.Smth.Controls.Elements
{
    ...
    {
        public static void AddContent(this RichEditControl richtxtContent, Thread thread)
        {
            if (thread != null && richtxtContent != null)
            {
                richtxtContent.BeginUpdate();
                Document doc = richtxtContent.Document;                
                ...
                string content = CommonUtil.ReplaceSpecialChars(thread.Content);
                {
                    ...
                    {
                        foreach (System.Text.RegularExpressions.Match mt in mtCollection)
                        {
                            ...
                            ///Add plain text.
                            {
                                ...
                                richtxtContent.Document.AppendText(tempContent);
                            ...
                            if (mt.Value.Contains("<b>"))
                            {
                                ...
                                int index = richtxtContent.Text.Length;
                                richtxtContent.Document.AppendText(tVal);
                            ...
                            if (mt.Groups["Type"].Value.ToString() == ThreadFactory.ImageToken)
                            {
                                ...
                                if (thread.Images.ContainsKey(url))
                                {
                                    ...
                                    //richtxtContent.Document.AppendHtmlText();//.InsertLink(data, url.Replace("/middle", ""), richtxtContent.Text.Length);
                                    richtxtContent.Document.Images.Insert(richtxtContent.Document.CaretPosition, thread.Images[url]);
                                }
                                else
                                {
                            ...
                            if (mt.Groups["Type"].Value.ToString() == ThreadFactory.IconToken)
                            {
                                ...
                                if (thread.Images.ContainsKey(url))
                                {
                                    richtxtContent.Document.Images.Insert(richtxtContent.Document.CaretPosition, thread.Icons[url]);
                                }
                                else
                                {
                                    //richtxtContent.Document.Images.Insert(RtfUtil.GetRtfCode("图片下载失败"),
                                    //                          url,
                                    //                          richtxtContent.Text.Length);
                                }
                            }
                            ...
                            if (mt.Groups["Type"].Value.ToString() == ThreadFactory.AnchorToken)
                            {
                                ...
                                Hyperlink link = richtxtContent.Document.Hyperlinks.Create(richtxtContent.Document.CaretPosition, thread.Anchors[anchorCounter].Text.Length);

                                link.NavigateUri = thread.Anchors[anchorCounter].Url;
                                link.Anchor = thread.Anchors[anchorCounter++].Text;
                                link.Target = "_blank";
                                link.Visited = false;

                                //.InsertLink(RtfUtil.GetRtfCode(thread.Anchors[anchorCounter].Text),
...
/*
                    ///Colored the replied thread content.
                    {
                        ...
                        {
                            foreach (System.Text.RegularExpressions.Match mt in mc)
                            {
